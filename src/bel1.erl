...
makeCodeTree(T1, T2) ->
  #fork{left = T1,
    ...
    chars = lists:append([chars(T1), chars(T2)])}.
...
addLetter([], C) -> [{C, 1}];
addLetter([{HeadChar, Freq} | T], C) when HeadChar =:= C -> [{HeadChar, Freq + 1} | T];
addLetter([H | T], C) -> [H | addLetter(T, C)].

-spec createFrequencies(list(char())) -> list({char(), non_neg_integer()}).
createFrequencies([H | []]) -> addLetter([], H);
createFrequencies([H | T]) -> addLetter(createFrequencies(T), H).
...
makeOrderedLeafList([{X, Y} | []]) -> [#leaf{char = X, weight = Y}];
makeOrderedLeafList([{X, Y} | [{X2, Y2} | T]]) when Y > Y2 ->
  [#leaf{char = X2, weight = Y2} | makeOrderedLeafList([{X, Y} | T])];
makeOrderedLeafList([{X, Y} | T]) -> [#leaf{char = X, weight = Y} | makeOrderedLeafList(T)].
...
combine([]) -> [];
combine([H | []]) -> [H];
combine([H | [H2 | T]]) -> [makeCodeTree(H, H2) | T].
%%combine([X | [Y | T]]) ->
%%  if
%%    weight(X) > weight(Y) ->
%%      [Y | combine([X | T])];
...
%%decode([HeadTree : leaf| Tree], [H | BitStream]) ->


decode(#fork{left = L} = Parent, [H,T]) -> [chars(L) | decode(Parent, [H|T])];

decode(CodeTree, BitStream) -> [{Bit, TreeNode} || Bit <- BitStream, Bit == 0,
                                                   TreeNode <- CodeTree].
